...
async function handleCreate() {
  ...
  try {
    /* Check if admin exists in cloud (listUsers) AND master vault exists.
       Both must exist for a valid workspace. */
    ...
    const hasVault = vault && vault.rev > 0;
    
    if (!anyAdmin || !hasVault) {
      /* No valid workspace → bootstrap admin */
      const user = await Store.bootstrapAdmin(u, p);
      await Sync.upsertUser(user);
      const vaultPayload = await Store.exportVaultPayload();
      await Sync.pushMasterVault(vaultPayload.payload, 0);
      clearLoginDraft();
