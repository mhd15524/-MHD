...
/* ---------- Typeable date inputs (display dd/mm/yyyy, store ISO) ---------- */
...
function parseInputDate(s) {
  ...
  if (!t) return '';
  const m = t.match(/^(\d{1,4})[\s\/\.\-](\d{1,2})[\s\/\.\-](\d{1,4})$/);
  if (!m) return '';
  const a = parseInt(m[1], 10), b = parseInt(m[2], 10), c = parseInt(m[3], 10);
  let y, mo, d;
  if (m[1].length === 4) { y = a; mo = b; d = c; }
  else if (m[3].length === 4) { y = c; mo = b; d = a; }
  else {
    d = a; mo = b; y = c < 100 ? (c < 50 ? 2000 + c : 1900 + c) : c;
    if (mo > 12) { const t2 = d; d = mo; mo = t2; }
  }
  const dt = new Date(y, mo - 1, d);
  if (dt.getFullYear() !== y || dt.getMonth() !== mo - 1 || dt.getDate() !== d) return '';
  return isoOf(dt);
}
function inputDateValue(el) { return parseInputDate(el && el.value); }
function dateVal(id) { const el = document.getElementById(id); return el ? parseInputDate(el.value) : ''; }

/* ============================================================
   LANGUAGE — English / Arabic toggle (whole-app UI translation)
   ------------------------------------------------------------
   The render templates stay in English; after each render a pass
   translates visible text via a phrase + word dictionary and flips
   the document direction for RTL. Data (names, notes, amounts)
   is never modified — only what is displayed.
   ============================================================ */
const LANG_KEY = 'il_lang';
let L = 'en';
try { L = localStorage.getItem(LANG_KEY) === 'ar' ? 'ar' : 'en'; } catch (e) {}
...
const AR_PHRASES = {
  ...
  'Installments Ledger': 'دفتر الأقساط',
  ...
  'Amount Collected': 'المبلغ المحصَّل',
  'Outstanding Balance': 'الرصيد المستحق',
  'Overdue Payments': 'الدفعات المتأخرة',
  'Overdue Balance': 'الرصيد المتأخر',
  'Due This Week': 'المستحق هذا الأسبوع',
  'Recent Customers': 'أحدث العملاء',
  'View all': 'عرض الكل',
  'No overdue payments. Great job!': 'لا توجد دفعات متأخرة. أحسنت!',
  'No installments due this week': 'لا توجد أقساط مستحقة هذا الأسبوع',
  'No customers yet — tap + to add one': 'لا يوجد عملاء بعد — اضغط + للإضافة',
  'No customers match your search.': 'لا يوجد عملاء يطابقون البحث.',
  'customers registered': 'عملاء مسجلون',
  'payments received': 'دفعات مستلمة',
  'still to be collected': 'متبقٍ للتحصيل',
  'customer(s) behind': 'عملاء متأخرون',
  'Add customer': 'إضافة عميل',
  'Paste above, then the installments appear here.': 'الصق النص بالأعلى وستظهر الأقساط هنا.',
  'No installments detected yet.': 'لم يتم اكتشاف أقساط بعد.',
  'Car Total': 'إجمالي السيارة',
  'Cash Paid': 'النقد المدفوع',
  ...
  'Amount per Installment': 'قيمة القسط',
  'Down Payment': 'الدفعة الأولى',
  'Balance to be paid': 'الرصيد المتبقي للدفع',
  ...
  'Confirm & Create': 'تأكيد والإنشاء',
  'Full Name *': 'الاسم الكامل *',
  'Optional notes about the customer': 'ملاحظات اختيارية عن العميل',
  'e.g. Ahmed Mohamed': 'مثال: أحمد محمد',
  'Total Amount *': 'المبلغ الإجمالي *',
  'Start Date *': 'تاريخ البدء *',
  'Number of Installments *': 'عدد الأقساط *',
  'Amount per Installment *': 'قيمة القسط *',
  'Review the details, then confirm to create the customer.': 'راجع التفاصيل ثم أكّد لإنشاء العميل.',
  ...
  'Data encrypted at rest — unlocked only with your password': 'البيانات مشفرة — تُفتح فقط بكلمة مرورك',
  'Create your account. The first account becomes the admin.': 'أنشئ حسابك. أول حساب يصبح مدير النظام.',
  'Sign in to unlock your encrypted ledger.': 'سجّل الدخول لفتح دفترك المشفر.',
  'Enter your username and password.': 'أدخل اسم المستخدم وكلمة المرور.',
  'Enter a username and password.': 'أدخل اسم مستخدم وكلمة مرور.',
  'Password must be at least 4 characters.': 'كلمة المرور يجب أن تكون 4 أحرف على الأقل.',
  'Passwords do not match.': 'كلمتا المرور غير متطابقتين.',
  'User not found. If this is your first time, click "Create an account" below.': 'المستخدم غير موجود. إذا كانت هذه أول مرة، اضغط "إنشاء حساب" بالأسفل.',
  'Cannot reach the sync server. Check your connection.': 'لا يمكن الوصول لخادم المزامنة. تحقق من اتصالك.',
  'Add New Role': 'إضافة دور جديد',
  'Role Name': 'اسم الدور',
  'Permissions': 'الصلاحيات',
  'Page Access': 'الوصول للصفحات',
  'Customer Access': 'الوصول للعملاء',
...
const AR_WORD_KEYS = Object.keys(AR_WORDS).sort(function (a, b) { return b.length - a.length; });
const AR_WORD_RE = {};
AR_WORD_KEYS.forEach(function (k) { AR_WORD_RE[k] = new RegExp('\\b' + k.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\b', 'g'); });

function translateText(text) {
  if (!text) return text;
  const s = String(text);
  if (AR_PHRASES[s]) return AR_PHRASES[s];
  let out = s;
  for (let i = 0; i < AR_WORD_KEYS.length; i++) {
    const k = AR_WORD_KEYS[i];
    out = out.replace(AR_WORD_RE[k], AR_WORDS[k]);
  }
  return out;
}
...
function applyLang() {
  ...
  const attrs = document.querySelectorAll('input[placeholder], textarea[placeholder], [title], [aria-label]');
  attrs.forEach(function (el) {
    if (el.hasAttribute('placeholder')) el.setAttribute('placeholder', translateText(el.getAttribute('placeholder')));
    if (el.hasAttribute('title')) el.setAttribute('title', translateText(el.getAttribute('title')));
    const al = el.getAttribute('aria-label');
    if (al) el.setAttribute('aria-label', translateText(al));
  });
}
...
function langBtn() {
  return '<button class="btn btn-ghost btn-sm lang-btn" id="lang_btn" onclick="toggleLang()" title="English / العربية">' + (L === 'ar' ? 'EN' : 'ع') + '</button>';
}
...
function toast(msg, type) {
  ...
  el.innerHTML = (type === 'ok' ? '✓ ' : type === 'err' ? '✕ ' : '') + escHtml(translateText(msg));
  root.appendChild(el);
  setTimeout(function () {
...
/* Red mark: customer has an installment due (today or earlier) that is unpaid. */
function hasUnpaidDue(c) {
  if (c.plan.type === 'cash') return false;
  if (balanceOf(c) <= 0.005) return false;
  return installmentsOf(c).some(function (it) {
    const s = instStatus(it, c);
...
function buildLoginForm(draft) {
  ...
  $('#app').innerHTML =
    ...
    toggleLink +
    '<p class="login-toggle" style="margin-top:8px;font-size:12px"><a href="#" onclick="event.preventDefault();forceResetLocal()" style="color:var(--muted-2)">⚠ Reset local cache</a></p>' +
    '</div></div>';
  const userEl = $('#l_user');
  if (userEl && draft) {
...
function loginError(msg) {
  ...
  if (el) { el.textContent = translateText(msg); el.classList.add('show'); }
}
  ...
  try {
    ...
    if (!res.ok) {
      if (res.error === 'User not found') {
        loginError('User not found. If this is your first time, click "Create an account" below.');
      } else {
...
async function handleCreate() {
  ...
  try {
    /* 1. Check if this specific username already exists in Supabase */
    let cloudUser = null;
    try { cloudUser = await Sync.fetchUser(u); } catch (e) { /* ignore network error */ }

    if (cloudUser) {
      if (cloudUser.status === 'pending') {
        loginError('This account exists but is awaiting admin approval.');
        if (btn) { btn.disabled = false; btn.textContent = 'Create Account'; }
        return;
      }
      if (cloudUser.status === 'blocked') {
        loginError('This account has been blocked by the admin.');
        if (btn) { btn.disabled = false; btn.textContent = 'Create Account'; }
        return;
      }
      /* Existing approved user — sign them in */
      const vault = await Sync.fetchMasterVault();
      const ok = await Store.loginWithUserRec(cloudUser, p, vault ? vault.payload : null);
      if (!ok) {
        loginError('An account with this username already exists. Wrong password?');
        if (btn) { btn.disabled = false; btn.textContent = 'Create Account'; }
        return;
      }
      clearLoginDraft();
      toast('Signed in to existing account.', 'ok');
      afterAuthenticated();
      return;
    }

    /* 2. Check if ANY admin already exists in Supabase */
    let allUsers = [];
    try { allUsers = await Sync.listUsers(); } catch (e) { /* ignore */ }
    const adminExists = allUsers.some(function (x) { return x.is_admin === true || x.is_admin === 1; });

    if (adminExists) {
      /* Admin already set up — register as a regular pending user */
      const newUser = await Store.signUp(u, p);
      await Sync.upsertUser(newUser);
      clearLoginDraft();
      loginMode = 'signin';
      buildLoginForm(u);
      toast('Account created! Please wait for admin approval before signing in.', 'ok');
      return;
    }

    /* 3. First user ever — become the admin */
    const user = await Store.bootstrapAdmin(u, p);
    await Sync.upsertUser(user);
    const vaultPayload = await Store.exportVaultPayload();
    await Sync.pushMasterVault(vaultPayload.payload, 1);
    await Sync.pushBusinessData(Store.getData());
    clearLoginDraft();
    toast('Admin account created. Welcome!', 'ok');
    afterAuthenticated();
  } catch (e) {
    console.error('Create account error:', e);
    loginError('Create failed: ' + e.message);
    if (btn) { btn.disabled = false; btn.textContent = 'Create Account'; }
  }
}

function forceResetLocal() {
  if (!confirm('This will clear all local cached data (NOT cloud data). You will need to sign in again. Continue?')) return;
  Store.clearAllLocalData();
  try { sessionStorage.clear(); } catch (e) {}
  loginMode = null;
  location.hash = '';
  buildLoginForm('');
  toast('Local cache cleared. Please sign in.', 'ok');
}
...
async function afterAuthenticated() {
  ...
  navigate('dashboard');
  render();
  /* pull latest cloud data, then refresh the view */
  try {
    await Sync.syncAfterAuth();
    loadData();
    render();
  } catch (e) { /* already rendered with local data */ }
}
...
/* ============================================================
   SHELL + ROUTING
   ============================================================ */
function navigate(hash) { closeSidebar(); location.hash = '#/' + hash; }

/* Mobile drawer sidebar — the desktop sidebar is shown on phones via a hamburger button. */
function openSidebar() {
  const sb = $('#sidebar');
  if (!sb) return;
  sb.classList.add('open');
  const bd = $('#sidebar_backdrop');
...
function render() {
  ...
  $('#app').innerHTML =
    ...
    '</div>' +
    '</aside>' +
    '<div class="sidebar-backdrop" id="sidebar_backdrop" onclick="closeSidebar()"></div>' +
    '<div class="main">' +
    '<header class="topbar"><button class="menu-btn" id="menu_btn" onclick="toggleSidebar()" aria-label="Menu">☰</button>' +
    '<div class="titles"><h2>' + escHtml(title) + '</h2>' + (crumb ? '<div class="crumb">' + crumb + '</div>' : '') + '</div>' +
    '<div class="actions">' + langBtn() + topActions + '</div></header>' +
    ...
    panel('Recent Customers', '<a class="muted" style="font-size:12px" href="#/customers">View all →</a>',
      recent.map(function (c) {
        ...
        return '<div class="list-row" onclick="navigate(\'customers/' + c.id + '\')">' +
          '<div class="avatar' + (hasUnpaidDue(c) ? ' avatar-overdue' : '') + '" style="width:38px;height:38px">' + initials(c.name) + '</div>' +
          '<div class="grow"><b>' + escHtml(c.name) + (hasUnpaidDue(c) ? '<span class="red-dot"></span>' : '') + '</b><small>balance ' + fmtMoney(balanceOf(c)) + '</small></div>' +
  ...
  $('#content').innerHTML =
    ...
    (list.map(function (c) {
      ...
      return '<tr class="row-main" onclick="navigate(\'customers/' + c.id + '\')">' +
        '<td data-label="Customer"><div class="flex" style="gap:10px"><div class="avatar' + (hasUnpaidDue(c) ? ' avatar-overdue' : '') + '" style="width:36px;height:36px">' + initials(c.name) + '</div>' +
        '<div><b>' + escHtml(c.name) + (hasUnpaidDue(c) ? '<span class="red-dot" title="Has unpaid installments"></span>' : '') + '</b><div class="muted" style="font-size:12px">Customer #' + c.id + '</div></div></div></td>' +
...
function renderCustomerDetail(id) {
  ...
  $('#content').innerHTML =
    ...
    /* Schedule */
    '<div class="panel mb"><div class="panel-head"><h3>Installment Schedule</h3>' +
    '<button class="btn btn-secondary btn-sm" onclick="openSchedulePasteModal(' + c.id + ')">📋 Paste Schedule</button></div><div class="tbl-wrap">' +
    ...
    (plan.type === 'cash'
      ...
      : insts.map(function (it) {
        ...
        return '<tr><td data-label="#">' + it.i + '</td>' +
          '<td data-label="Amount" class="num"><span class="inst-amt">' + fmtMoney(eff) + '</span>' + (hasDiscount ? '<span class="inst-disc">disc −' + fmtMoney(instDiscount(it, c)) + '</span>' : '') + '<span class="inst-day">' + weekdayOf(it.due) + ' · <input type="text" inputmode="numeric" maxlength="10" placeholder="DD/MM/YYYY" class="inst-date date-input" title="Type a date as day/month/year — the schedule re-arranges by date" value="' + escAttr(fmtInputDate(it.due)) + '" onchange="setInstallmentDate(' + c.id + ',' + it.i + ',inputDateValue(this))"></span></td>' +
...
function setInstallmentDate(id, instNum, date) {
  if (!date) { toast('Enter a valid date as day/month/year, e.g. 15/08/2026.', 'err'); render(); return; }
  const data = Store.getData();
  const cust = data.customers.find(function (x) { return x.id === id; });
  if (!cust) return;
...
/* ============================================================
   PASTE SCHEDULE — auto-fill installment dates from a pasted text
   e.g.  "باقي 1550 10 17/5/2025 / 10 31/5 / 10 7/6/2025"
   Missing years are inferred (the next occurrence after the last date).
   ============================================================ */
function inferScheduleDate(d, m, y, prevISO) {
  let yy;
  if (y) {
    yy = parseInt(y, 10);
    if (yy < 100) yy += yy < 50 ? 2000 : 1900;
    return parseInputDate(d + '/' + m + '/' + yy);
  }
  const today = todayISO();
  yy = prevISO ? parseInt(prevISO.slice(0, 4), 10) : parseInt(today.slice(0, 4), 10);
  for (let k = 0; k < 3; k++) {
    const iso = parseInputDate(d + '/' + m + '/' + (yy + k));
    if (!iso) continue;
    if (!prevISO) {
      if (iso >= today) return iso;
      continue;
    }
    if (iso > prevISO) return iso;
  }
  return '';
}
...
function parseScheduleText(text) {
  ...
  String(text || '').split('\n').forEach(function (line) {
    line = line.trim();
    if (!line) return;
    let parts = null, amt = null;
    if (/^باقي/.test(line)) {
      /* "باقي 1550 10 17/5/2025" → remaining balance, then amount + date */
      const rem = line.match(/^باقي\s*(\d+(?:[.,]\d+)?)/);
      if (rem && !remaining) remaining = parseFloat(rem[1].replace(',', '.'));
      line = line.replace(/^باقي\s*/, '').replace(/^\d+(?:[.,]\d+)?\s*/, '');
      if (!line) return;
    }
    const ad = line.match(/^(\d+(?:[.,]\d+)?)\s+(\d{1,2})[\/\-.](\d{1,2})(?:[\/\-.](\d{2,4}))?$/);
    if (ad) { amt = parseFloat(ad[1].replace(',', '.')); parts = [ad[2], ad[3], ad[4] || '']; }
    else {
      const jd = line.match(/^(\d{1,2})[\/\-.](\d{1,2})(?:[\/\-.](\d{2,4}))?$/);
      if (jd) { amt = lastAmt; parts = [jd[1], jd[2], jd[3] || '']; }
      else return;
    }
    const iso = inferScheduleDate(parseInt(parts[0], 10), parseInt(parts[1], 10), parts[2], prevISO);
    if (iso) {
      lastAmt = amt;
      prevISO = iso;
      insts.push({ due: iso, amount: round2(amt) });
    }
  });
  insts.sort(function (a, b) { return a.due === b.due ? 0 : (a.due < b.due ? -1 : 1); });
  return { insts: insts, remaining: round2(remaining) };
}

function previewSchedule(text, elId) {
  const parsed = parseScheduleText(text);
  const insts = parsed.insts;
  const el = document.getElementById(elId);
  if (!el) return;
  if (!insts.length) { el.innerHTML = '<span class="muted">No installments detected yet.</span>'; return; }
  const shown = insts.slice(0, 12).map(function (x, i) {
    return '<div>' + (i + 1) + '. <b>' + fmtMoney(x.amount) + '</b> — ' + fmtDate(x.due) + '</div>';
  }).join('');
  const more = insts.length > 12 ? '<div class="muted" style="margin-top:4px">… +' + (insts.length - 12) + ' more</div>' : '';
  el.innerHTML = '<div class="muted" style="margin-bottom:4px">✓ ' + insts.length + ' installment(s)' + (parsed.remaining ? ' · balance ' + fmtMoney(parsed.remaining) : '') + ':</div>' + shown + more;
}

function openSchedulePasteModal(id) {
  const data = Store.getData();
  const cust = data.customers.find(function (x) { return x.id === id; });
  if (!cust) return;
  openModal('📋 Paste Schedule — ' + cust.name,
    '<div class="modal-notice">Paste the payment schedule text. The app <b>auto-fills the dates</b> — missing years are guessed from the previous date (e.g. <b>10 31/5</b> → <b>31/5/2025</b>).</div>' +
    '<div class="field"><label>Paste schedule here</label><textarea id="sc_text" rows="12" placeholder="باقي 1550 10 17/5/2025\n10 24/5/2025\n10 31/5\n10 7/6/2025"></textarea></div>' +
    '<div class="field"><label>Preview</label><div class="muted" id="sc_preview" style="font-size:12.5px">Paste above, then the installments appear here.</div></div>',
    '<button class="btn btn-secondary" onclick="closeModal()">Cancel</button>' +
    '<button class="btn btn-primary" onclick="applySchedulePaste(' + id + ')">✓ Apply Schedule</button>');
  const ta = document.getElementById('sc_text');
  if (ta) ta.oninput = function () { previewSchedule(this.value, 'sc_preview'); };
}

function applySchedulePaste(id) {
  const ta = document.getElementById('sc_text');
  const parsed = parseScheduleText(ta ? ta.value : '');
  const insts = parsed.insts;
  if (!insts.length) { toast('No valid installments found. Paste like: 10 17/5/2025', 'err'); return; }
  const data = Store.getData();
  const cust = data.customers.find(function (x) { return x.id === id; });
  if (!cust) return;
  const clean = storeInstallments(cust, insts);
  cust.plan.type = 'installments';
  cust.plan.amountPerInstallment = round2(clean[0].amount);
  if (!(cust.plan.total > 0)) cust.plan.total = parsed.remaining > 0 ? parsed.remaining : round2(clean.reduce(function (s, x) { return s + x.amount; }, 0));
  Store.saveData(data);
  closeModal();
  toast('Schedule applied — ' + clean.length + ' installments, dates filled in.', 'ok');
  render();
}
...
function step2Html() {
  ...
  return '<div class="wizard-dots"><div class="dot on"></div><div class="dot on"></div><div class="dot"></div></div>' +
    ...
    '<div class="field"><label>Start Date *</label><input type="text" inputmode="numeric" maxlength="10" placeholder="DD/MM/YYYY" class="date-input" id="w_start" value="' + fmtInputDate(s.startDate) + '"></div>' +
...
function wizardNext() {
  ...
  if (wizard.step === 2) {
    ...
    const start = dateVal('w_start');
    if (!(total > 0)) { toast('Enter the total amount.', 'err'); return; }
    if (!start) { toast('Enter a valid start date (day/month/year).', 'err'); return; }
    let count;
...
function openPaymentModal(id) {
  ...
  openModal('Record Payment — ' + c.name,
    ...
    '<div class="field-row">' +
    '<div class="field"><label>Date</label><input type="text" inputmode="numeric" maxlength="10" placeholder="DD/MM/YYYY" class="date-input" id="p_date" value="' + fmtInputDate(todayISO()) + '"></div>' +
...
function applyInstPlus(id, instNum) {
  const c = getCustomer(id);
  if (!c) return;
  const date = dateVal('p_date');
  const amount = parseFloat(wVal('p_amount'));
  const note = wVal('p_note').trim();
  if (!date) { toast('Enter a valid date as day/month/year, e.g. 15/08/2026.', 'err'); return; }
...
function openEditModal(id) {
  ...
  if (planEditable && plan.type !== 'cash') {
    planFields =
      ...
      '<div class="row"><div class="field"><label>Start Date</label><input type="text" inputmode="numeric" maxlength="10" placeholder="DD/MM/YYYY" class="date-input" id="e_start" value="' + fmtInputDate(plan.startDate) + '"></div>' +
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  /* Pressing Enter inside a typeable date field commits the value (blur fires the change handler). */
  document.addEventListener('keydown', function (e) {
    if (e.key === 'Enter' && e.target && e.target.matches && e.target.matches('input.date-input')) {
      e.preventDefault();
      e.target.blur();
    }
  });
  ...
  (async function () {
    try {
      ...
      if (ok && Store.currentUser()) {
        render();
        try { await Sync.syncAfterAuth(); loadData(); render(); } catch (e) {}
      } else renderLogin();
    } catch (e) {
      renderLogin();
    }
