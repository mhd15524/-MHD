...
const PDF = (function () {
  ...
  function buildToUnicode(used) {
    ...
    /* Chunked so each beginbfchar block matches its entry count (max 100/block). */
    for (let k = 0; k < used.length; k += 100) {
      const slice = used.slice(k, k + 100);
      s += slice.length + ' beginbfchar\n';
  ...
  function buildStatement(c, settings) {
    ...
    const count = isCash ? 1 : (plan.count || 0);
    ...
    infoBox(ML + infoBoxW + gap, y, 'NUMBER OF INSTALLMENTS', String(count), 0);
