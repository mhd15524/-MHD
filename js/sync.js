...
const Sync = (function () {
  const CFG = {
    url: 'https://cgaopjqydylfhkgnqegf.supabase.co',
  ...
  async function fetchMasterVault() {
    const url = CFG.url + '/rest/v1/' + CFG.tables.vaults +
      '?owner=eq.master&select=*&limit=1';
  ...
  async function fetchUser(username) {
    const url = CFG.url + '/rest/v1/' + CFG.tables.users +
      '?username=eq.' + encodeURIComponent(username) + '&select=*&limit=1';
    const rows = await fetchJson(url);
    return rows && rows.length ? rows[0] : null;
  }

  async function listUsers() {
    const url = CFG.url + '/rest/v1/' + CFG.tables.users + '?select=*&order=created_at.desc';
  ...
  async function fetchRoles() {
    const url = CFG.url + '/rest/v1/' + CFG.tables.roles + '?select=*';
    return await fetchJson(url) || [];
  }

  async function upsertRole(rec) {
    const url = CFG.url + '/rest/v1/' + CFG.tables.roles;
    const headers = { ...authHeaders(), Prefer: 'resolution=merge-duplicates' };
    await fetch(url, { method: 'POST', headers, body: JSON.stringify([rec]) });
  }
