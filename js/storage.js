...
const Store = (function () {
  ...
  function clearAllLocalData() {
    [K_USERS, K_DATA, K_SESSION, K_KEY, K_ROLES].forEach(function (k) {
  ...
  async function bootstrapAdmin(username, password) {
    const users = getUsers();
    // Allow re-bootstrap only if there is no OTHER admin (different username)
    const otherAdmin = users.find(u => u.isAdmin && u.username !== username);
    if (otherAdmin) throw new Error('Admin already exists');
    ...
    // Replace any existing record for this username
  ...
  async function verifyCloudUser(userRec, password) {
    ...
    if (!userRec.wrappedKey) throw new Error('Account not yet approved — no workspace key assigned.');
  ...
  async function loginWithUserRec(userRec, password, masterPayload) {
    ...
    let data = null;
    if (masterPayload) {
      data = await decryptObj(dataKey, masterPayload);
    }
    if (!data) {
      /* No vault yet - use local data or default */
      data = mem || defaultData();
    }
  ...
  /* Set mem directly from plain Supabase table data (no decryption needed). */
  async function importBusinessData(data) {
    if (!data) return false;
    mem = data;
    if (memKey) await writeData(memKey, mem);
    return true;
  }
  ...
  return {
    ...
    setSession, getSession, clearSession, restoreSession, clearAllLocalData,
    bootstrapAdmin, signUp, loginWithUserRec, changePassword,
    unwrapWorkspaceKey, wrapKeyForUser,
    exportVaultPayload, importVaultPayload, importBusinessData,
    setOnSave
  };
})();